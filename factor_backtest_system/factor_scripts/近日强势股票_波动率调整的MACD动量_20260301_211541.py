...
因子说明:
用MACD值除以20日波动率进行风险调整，识别在相对低波动环境下出现明确趋势信号的强势股。
...
FACTOR_DEFINITION = {
    ...
    "tools": [
        ...
        },
        {
            ...
            "params": {
                "values": "macd_val / (volatility_20 + 0.0001)"
            },
            "var": "macd_vol_adj"
        }
    ],
    "expression": "macd_vol_adj",
    "rationale": "用MACD值除以20日波动率进行风险调整，识别在相对低波动环境下出现明确趋势信号的强势股。"
}
# ==================== 因子定义结束 ====================
...
class 波动率调整的macd动量Calculator:
    ...
    def calculate(self, trade_date: str) -> pd.DataFrame:
        ...
        # 工具 1: macd
        print(f"   [1/3] 执行工具: macd")
        macd_val = execute_tool(
            tool_name='macd',
            data=data,
            params={"values": "收盘价", "fast": 12, "slow": 26, "signal": 9},
            computed_vars=self.computed_vars
        )
        self.computed_vars['macd_val'] = macd_val
        print(f"      ✅ macd_val 计算完成")

        # 工具 2: rolling_std
        print(f"   [2/3] 执行工具: rolling_std")
        ...
        macd_vol_adj = execute_tool(
            tool_name='zscore_normalize',
            data=data,
            params={"values": "macd_val / (volatility_20 + 0.0001)"},
            computed_vars=self.computed_vars
        )
        self.computed_vars['macd_vol_adj'] = macd_vol_adj
        print(f"      ✅ macd_vol_adj 计算完成")

        
        # 3. 计算因子表达式
        print(f"\n📐 计算因子表达式...")
        print(f"   原始表达式: macd_vol_adj")
        
        # 解析表达式
        parsed_expr = ExpressionParser.parse_expression("macd_vol_adj")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='波动率调整的MACD动量 - 因子计算脚本')
    ...
    # 创建因子计算器
    calculator = 波动率调整的macd动量Calculator()
