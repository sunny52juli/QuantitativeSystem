...
因子说明:
结合5日动量与价格接近20日高点的突破特征，标准化后识别强势启动信号。
...
FACTOR_DEFINITION = {
    ...
    "tools": [
        ...
        },
        {
            ...
            "params": {
                "values": "mom5 * (收盘价 / (high_20 + 0.0001))"
            },
            "var": "momentum_breakout"
        }
    ],
    "expression": "momentum_breakout",
    "rationale": "结合5日动量与价格接近20日高点的突破特征，标准化后识别强势启动信号。"
}
# ==================== 因子定义结束 ====================
...
class 动量突破因子Calculator:
    ...
    def calculate(self, trade_date: str) -> pd.DataFrame:
        ...
        self.computed_vars['mom5'] = mom5
        print(f"      ✅ mom5 计算完成")

        # 工具 2: rolling_max
        print(f"   [2/3] 执行工具: rolling_max")
        high_20 = execute_tool(
            tool_name='rolling_max',
            data=data,
            params={"values": "收盘价", "window": 20},
            computed_vars=self.computed_vars
        )
        self.computed_vars['high_20'] = high_20
        print(f"      ✅ high_20 计算完成")

        # 工具 3: zscore_normalize
        print(f"   [3/3] 执行工具: zscore_normalize")
        momentum_breakout = execute_tool(
            tool_name='zscore_normalize',
            data=data,
            params={"values": "mom5 * (收盘价 / (high_20 + 0.0001))"},
            computed_vars=self.computed_vars
        )
        self.computed_vars['momentum_breakout'] = momentum_breakout
        print(f"      ✅ momentum_breakout 计算完成")

        
        # 3. 计算因子表达式
        print(f"\n📐 计算因子表达式...")
        print(f"   原始表达式: momentum_breakout")
        
        # 解析表达式
        parsed_expr = ExpressionParser.parse_expression("momentum_breakout")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='动量突破因子 - 因子计算脚本')
