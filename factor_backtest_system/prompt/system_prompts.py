...
⚠️ 除非了解系统架构，否则不建议修改本文件。
    如需自定义业务内容，请修改 user_prompt 文件夹中的配置。
"""
...
class StrategyPrompts:
    """策略模板 Prompts（从 user_prompt.STRATEGY_CONFIGS 加载）"""

    # 动态加载策略配置
    _strategies = dict(STRATEGY_CONFIGS)
...
for _name, _prompt in STRATEGY_CONFIGS.items():
    setattr(StrategyPrompts, _name, _prompt)


class AIFactorMinerPrompts:
    """AI 因子挖掘器 Prompts（从 user_prompt 加载）"""

    SYSTEM_PROMPT_TEMPLATE = SYSTEM_ROLE
    USER_PROMPT_TEMPLATE = USER_REQUEST_FORMAT
    FALLBACK_SKILL_CONTENT = FALLBACK_SKILL_CONTENT


class FactorMiningAgentPrompts:
    """因子挖掘代理 Prompts（从 user_prompt 加载）"""
...
class MessageTemplates(BaseMessageMixin):
    ...
    SUCCESS_MESSAGES = {
        **COMMON_SUCCESS_MESSAGES,
        ...
        'skill_loaded': "✅ 成功从 SKILL.md 加载技能文档",
    }

    # 警告消息：通用 + 本系统特有（覆盖通用的部分描述）
    WARNING_MESSAGES = {
        **COMMON_WARNING_MESSAGES,
        'no_api_key': "⚠️ 未找到 DEFAULT_API_KEY，回退到规则生成",
        'ai_mode_disabled': "⚠️ AI Agent 模式：未启用（使用规则生成）",
        'nan_values': "⚠️ 变量 {var_name} 包含 {count} 个 NaN 值 ({percentage:.1f}%)",
        'partial_nan': "⚠️ 提示：前{count}个值为 NaN（可能是滚动窗口导致）",
        'all_nan': "❌ 警告：所有值都是 NaN！",
        'invalid_format': "⚠️ 返回格式不是列表，回退到规则生成",
        'no_valid_factors': "⚠️ 没有有效因子，回退到规则生成",
        'skill_not_found': "SKILL.md 文件未找到，使用简化技能文档作为 fallback",
    }
    ...
    INFO_MESSAGES = {
        **COMMON_INFO_MESSAGES,
        ...
        'agent_started': "🤖 AI 因子挖掘器 已启动（MCP 工具集成）",
        'agent_initialized': "✅ 因子挖掘代理初始化完成",
        'generating_factors': "🤖 Agent 正在生成因子...",
        ...
        'using_api': "🤖 使用 AI Agent 生成因子...",
        'parsed_factor': "✅ 解析因子：{name}",
        'ai_enabled': "✅ AI Agent 模式：已启用（使用 DeepSeek API）",
    ...
    HINT_MESSAGES = {
        **COMMON_HINT_MESSAGES,
        'set_api_key': "提示：设置 DEFAULT_API_KEY 环境变量以启用 AI 生成",
        'nan_reasons': """可能原因:
      1. 滚动窗口大于数据长度
      2. 输入数据包含 NaN
      3. 数学运算产生 NaN（如除零）
