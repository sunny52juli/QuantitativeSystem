...
⚠️ 除非了解系统架构，否则不建议修改本文件。
    如需自定义业务内容，请修改 asking_config.py。
"""

import logging
from typing import List

from . import asking_config as _config
...
class StockQueryPrompts(BaseMessageMixin):
    """
    股票查询 Prompts（从 asking_config 加载）
    ...
    _query_examples = dict(_config.QUERY_EXAMPLES)

    # 系统模板（从 config 加载）
    SYSTEM_PROMPT_TEMPLATE = _config.SYSTEM_ROLE
    USER_QUERY_TEMPLATE = _config.USER_QUERY_FORMAT
    ...
    ERROR_MESSAGES = {
        **COMMON_ERROR_MESSAGES,
        'no_api_key': "❌ 未检测到API密钥，请设置环境变量 DEFAULT_API_KEY",
        'query_failed': "❌ 查询失败: {error}",
        'no_results': "📭 未找到符合条件的股票",
        'invalid_response': "⚠️ AI返回格式无效，请重试",
    ...
    WARNING_MESSAGES = {
        **COMMON_WARNING_MESSAGES,
        ...
        'tool_unavailable': "⚠️ MCP工具 {tool_name} 不可用，使用备用方案",
    }
    ...
    HINT_MESSAGES = {
        **COMMON_HINT_MESSAGES,
        'set_api_key': "💡 提示: 设置DEFAULT_API_KEY环境变量以启用AI分析",
    ...
    def get_system_prompt(cls) -> str:
        ...
        return cls.SYSTEM_PROMPT_TEMPLATE.format(skill_content=_config.SKILL_CONTENT)
    ...
    def get_demo_queries(cls) -> List[str]:
        """
        获取演示查询列表（从 asking_config.QUERY_EXAMPLES 加载）

        Returns:
            查询字符串列表，顺序与 asking_config.QUERY_EXAMPLES 定义顺序一致
        """
...
for _name, _query in _config.QUERY_EXAMPLES.items():
    if _name in _RESERVED_ATTRS:
        logger.warning(
            f"查询示例键名 '{_name}' 与 StockQueryPrompts 已有属性冲突，已跳过注入。"
            f"请在 asking_config.py 中修改该键名。"
        )
        continue
