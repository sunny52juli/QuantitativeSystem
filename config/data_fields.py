...
class DataFields:
    ...
    # 必需字段（数据验证时检查）
    REQUIRED_FIELDS = ['date', 'code', 'open', 'high', 'low', 'close', 'vol']  # 修复：vol改为vol
    
    # 数值字段（需要数值类型）
    NUMERIC_FIELDS = [
        'open', 'high', 'low', 'close',
        'vol', 'amount',  # 修复：vol改为vol
        'vwap', 'pct_change', 'ret', 'ret1', 'volatility'
    ]
    ...
    def get_field_type(cls, field_name: str) -> str:
        ...
        if normalized in cls.PRICE_FIELDS.values():
            return 'price'
        elif normalized in cls.vol_FIELDS.values():
            return 'vol'
        elif normalized in cls.INFO_FIELDS.values():
            return 'info'
        elif normalized in cls.DERIVED_FIELDS.values():
            return 'derived'
        elif normalized in cls.CATEGORY_FIELDS.values():
            return 'category'
        else:
            return 'unknown'
    ...
    def validate_fields_in_expression(cls, expression: str) -> Dict[str, List[str]]:
        ...
        # 排除常见的函数名和关键字
        exclude_words = {
            'abs', 'log', 'sqrt', 'max', 'min', 'sum', 'mean', 'std',
            'rolling', 'pct', 'lag', 'rank', 'zscore', 'clip',
            'and', 'or', 'not', 'if', 'else', 'for', 'in'
        }
        english_fields = [
            word for word in re.findall(r'\b[a-z_]+\b', expression)
...
def print_field_summary():
    ...
    categories = {
        ...
        '估值指标': ['turnover_rate', 'turnover_rate_f', 'vol_ratio', 'pe', 'pe_ttm', 'pb', 'ps', 'ps_ttm', 'dv_ratio', 'dv_ttm', 'total_share', 'float_share', 'free_share', 'total_mv', 'circ_mv'],
        '复权因子': ['adj_factor'],
        '资金流向': ['buy_sm_vol', 'buy_sm_amount', 'sell_sm_vol', 'sell_sm_amount', 'buy_md_vol', 'buy_md_amount', 'sell_md_vol', 'sell_md_amount', 'buy_lg_vol', 'buy_lg_amount', 'sell_lg_vol', 'sell_lg_amount', 'buy_elg_vol', 'buy_elg_amount', 'sell_elg_vol', 'sell_elg_amount', 'net_mf_vol', 'net_mf_amount'],
